...
test('logs the bigger boat quote when the button is clicked', async () => {
  const log = jest.spyOn(console, 'log').mockImplementation(() => {});

  render(<App />);
  await userEvent.click(
    screen.getByRole('button', { name: /you're gonna need a bigger boat/i })
  );

  expect(log).toHaveBeenCalledWith("You're gonna need a bigger boat.");
  log.mockRestore();
});
